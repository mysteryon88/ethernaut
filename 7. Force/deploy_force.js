...
async function main() {
  ...
  const hackForce = await HackForce.deploy({value: ethers.utils.parseUnits("1.0", "ether")});
  await hackForce.deployed();
  console.log(`HackForce deployed to ${hackForce.address}`);

  console.log(`HackForce balance = ${
  ...
  console.log(`New HackForce balance = ${
    ethers.utils.formatEther(
      await ethers.provider.getBalance(hackForce.address)
      )
  } ETH`);
  console.log(`New Force balance = ${
    ethers.utils.formatEther(
      await ethers.provider.getBalance(force.address)
      )
  } ETH`);
